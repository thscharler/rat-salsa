...
impl<'a> EndlessScroll<'a> {
    ...
    /// Style when focused.
    fn focus_style(mut self, style: impl Into<Style>) -> Self {
        self.focus_style = Some(style.into());
        self
    }

    /// Base style.
    fn cursor_style(mut self, style: Style) -> Self {
        self.cursor_style = Some(style);
        self
    }

    fn v_scroll(mut self, scroll: Scroll<'a>) -> Self {
