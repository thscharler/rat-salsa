...
pub mod core {
    ...
    impl Mask {
        ...
        fn can_overwrite(&self, c: &str) -> bool {
            match self {
                Mask::Digit0(_) | Mask::Numeric(_) => false,
                Mask::DecimalSep => "." == c,
                Mask::GroupingSep => false,
                Mask::Sign => "-" == c || " " == c,
                Mask::Hex0 => c == "0",
    ...
    impl InputMaskCore {
        ...
        pub fn advance_cursor(&mut self, c: char) {
            ...
            loop {
                    ...
                    && self.is_valid_c(&mask.right, c)
                {
                    ...
                    mask.peek_left,
                    Mask::Digit0(EditDirection::Ltor) | Mask::Numeric(EditDirection::Ltor)
        ...
        fn insert_sign(&mut self, c: char) -> Result<bool, fmt::Error> {
            ...
            let (b, c0, p, c1, a) = grapheme::split_mask_match(&self.value, "-", mask.nr_range());
            if p == "-" {
                let mut buf = String::new();
                buf.push_str(b);
                buf.push_str(c0);
            ...
            if c == self.neg_sym() {
                for i in mask.nr_range() {
                    ...
                    if matches!(
                        ...
                        let (b, c0, c1, a) = grapheme::split_mask(self.value(), i, mask.nr_range());
    ...
    pub fn parse_mask(mask_str: &str) -> Result<Vec<MaskToken>, fmt::Error> {
        ...
        for m in mask_str.graphemes(true).chain(once("")) {
            ...
            match mask {
                Mask::Digit0(_) | Mask::Numeric(_) | Mask::GroupingSep | Mask::Sign => {
                    // no change
                }
                Mask::DecimalSep => {
