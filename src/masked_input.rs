mod mask {
    ...
    impl Mask {
        ...
        #[inline]
        pub(super) fn can_skip(&self, c: &str) -> bool {
            match self {
                Mask::Digit0(_) => false,
                Mask::Digit(_) => c == " ",
                Mask::Numeric(_) => c == " ",
        ...
        #[inline]
        pub(super) fn disp_value(&self) -> &str {
            match self {
                Mask::Digit0(_) => "0",
                Mask::Digit(_) => " ",
                Mask::Numeric(_) => " ",
    ...
    impl MaskToken {
        ...
        pub(super) fn nr_range(&self) -> Range<upos_type> {
            self.nr_start..self.nr_end
        }

        /// Range as Range.
        pub(super) fn range(&self) -> Range<upos_type> {
...
mod masked {
    ...
    pub struct MaskedCore {
        ...
        // tmp for rendering according to locale.
        rendered: String,
    ...
    impl MaskedCore {
        ...
        pub fn compact_value(&self) -> String {
            ...
            for (c, m) in self
                .masked
                .text_graphemes(TextPosition::new(0, 0))
                .expect("valid_position")
                .zip(self.mask.iter())
            {
                if !m.right.can_skip(c.grapheme()) {
                    buf.push_str(c.grapheme());
                }
            }
            buf
        }
        ...
        pub(super) fn render_value(&mut self) {
            self.rendered.clear();

            let text = self.masked.text().as_str();
            let mut idx = 0;
            loop {
                let mask = &self.mask[idx as usize];
                ...
                let sec = self
                    .masked
                    .str_slice(TextRange::new((mask.sec_start, 0), (mask.sec_end, 0)))
                    .expect("valid_range");
                let sec_mask = &self.mask[mask.sec_start as usize..mask.sec_end as usize];
        ...
        /// Create the rendered value.
        #[allow(unused_variables)]
        pub(super) fn render_condensed_value(&mut self) {
            self.rendered.clear();
    ...
    impl MaskedCore {
        ...
        pub fn remove_range(&mut self, range: Range<upos_type>) {
            if range.is_empty() {
                return;
            }
