...
fn enc(c: char, buf: &mut String) -> Result<(), TextError> {
    match c {
        '0'..='9' => buf.push(c),
        'a'..='z' => buf.push_str(format!("{}", (c as u32 - 'a' as u32) + 10).as_str()),
        'A'..='Z' => buf.push_str(format!("{}", (c as u32 - 'A' as u32) + 10).as_str()),
        ' ' => { /* noop */ }
        _ => return Err(TextError::InvalidValue),
    }
    Ok(())
}

fn valid_check_sum(iban: &str) -> Result<bool, TextError> {
    let mut cit = iban.chars();
    let Some(c0) = cit.next() else {
        return Err(TextError::InvalidValue);
    };
...
fn iban_len(iban: &str) -> Result<u8, TextError> {
    let cc = country_code(iban);
    for v in IBAN {
        if v.0 == cc {
            return Ok(v.1);
...
impl IBANInputState {
    ...
    /// Set the IBAN
    pub fn set_value(&mut self, iban: impl AsRef<str>) -> Result<(), TextError> {
    ...
    pub fn value(&mut self) -> Result<String, TextError> {
        let txt = self.widget.text();
        Ok(from_display_format(&self.country, txt))
    }
}

impl HandleEvent<crossterm::event::Event, Regular, TextOutcome> for IBANInputState {
...
impl HandleEvent<crossterm::event::Event, ReadOnly, TextOutcome> for IBANInputState {
    fn handle(&mut self, event: &crossterm::event::Event, _keymap: ReadOnly) -> TextOutcome {
        self.widget.handle(event, ReadOnly)
    }
}

impl HandleEvent<crossterm::event::Event, MouseOnly, TextOutcome> for IBANInputState {
